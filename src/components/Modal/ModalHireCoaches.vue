<template>
  <div>
    <div class="modal" id="hireModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <div class="modalBorderCustom modalBackground modal-content">
          ...
          <div class="modal-body">
            <div class="moda-body-hire-coach">
              <div @click="hireCoach(1)" class="modal-quantity-btn cursor-pointer">
                1 <span>({{ usd20 }} MDC)</span>
              </div>
              <div @click="hireCoach(5)" class="modal-quantity-btn cursor-pointer">
              ...
              <!-- <div @click="hireCoach(20)" class="modal-quantity-btn cursor-pointer">
  ...
  <ModalLoadingHireCoach ref="ModalLoadingHireCoach" />
</template>
<script>
import Web3 from 'web3';
import { Modal } from 'bootstrap'
import ModalLoadingHireCoach from './../../components/Modal/Loading/ModalLoadingHireCoach.vue'

export default {
  name: 'ModalHireCoaches',
  props: {
  },
  inject: {
    Mint: {
  ...
  methods: {
    showHireModal() {
      this.modal = new Modal(document.getElementById('hireModal'), {
        keyboard: false
      })
      var modalToggle = document.getElementById('hireModal')
    ...
    async hireCoach(amount) {
      this.$refs.ModalLoadingHireCoach.showModal();
      ...
      const result = await this.Mint().mintCoach(this.walletAddress, amount, this.referralAddress);
      if( result === true) {
        this.$emit('reveal-modal');
      }
      this.$refs.ModalLoadingHireCoach.hideModal();
    },
